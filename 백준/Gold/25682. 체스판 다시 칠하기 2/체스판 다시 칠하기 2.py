...
case2 = [[0]*(M+1) for _ in range(N+1)] #WBWBWBWB
        ...
        if i%2!=0 :
            if j%2!=0 :
                if m[i-1][j-1] == "B" :
                    ...
                    case2[i][j] += case2[i-1][j] + case2[i][j-1] - case2[i-1][j-1] + 1
                else :
...
for i in range(N-K+1) :
    for j in range(M-K+1) :
        comp = min(case1[i+K][j+K]-case1[i+K][j]-case1[i][j+K]+case1[i][j], case2[i+K][j+K]-case2[i+K][j]-case2[i][j+K]+case2[i][j])
